...
def index(request):
    ...
    if request.method == "POST":
        ...
        pt=request.POST.get('qo','')
        r=encyt(key,pt)
        ab="For Plain Text:- "+pt+"\n"
        cd="       For key:- "+key
        ef="       The Cipher Text is:- "+r
        return HttpResponse(ab+"<br/>"+cd+"<br/>"+ef)    

def index1(request):
    
    if request.method == "POST":
        key=request.POST.get('p','')
        pt=request.POST.get('po','')
        r=decyt(key,pt)
        #data={"Result:-":r}
        ba="For Encrypted Text:- "+pt
        dc="For key:- "+key
        fe="The Plain Text is:- "+r
        return HttpResponse(ba+"<br/>"+dc+"<br/>"+fe)    
        #return render (request,'home.html',{'ent':data})   
